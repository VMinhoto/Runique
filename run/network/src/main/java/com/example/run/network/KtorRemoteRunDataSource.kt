package com.example.run.network

import com.example.core.data.networking.constructRoute
import com.example.core.data.networking.delete
import com.example.core.data.networking.get
import com.example.core.data.networking.safeCall
import com.example.core.domain.run.RemoteRunDataSource
import com.example.core.domain.run.Run
import com.example.core.domain.util.DataError
import com.example.core.domain.util.EmptyResult
import com.example.core.domain.util.Result
import com.example.core.domain.util.map
import io.ktor.client.HttpClient
import io.ktor.client.request.forms.formData
import io.ktor.client.request.forms.submitFormWithBinaryData
import io.ktor.http.Headers
import io.ktor.http.HttpHeaders
import io.ktor.http.HttpMethod
import kotlinx.serialization.encodeToString
import kotlinx.serialization.json.Json

/**
 * Remote Run Data source implementation using ktor library
 * @param httpClient the client to make http calls (from ktor)
 */
class KtorRemoteRunDataSource(
    private val httpClient: HttpClient
): RemoteRunDataSource {

    /**
     * Function to get runes from remote database. Queries /runs endpoint. Gets the resulting json
     * and returns a run domain model.
     * @return [Result] with a list of runs or a network error
     */
    override suspend fun getRuns(): Result<List<Run>, DataError.Network> {
        return httpClient.get<List<RunDto>>(
            route = "/runs",
        ).map { runDtos->
            runDtos.map { it.toRun() }
        }
    }

    /**
     * Function to post runs to a remote database. Makes a post with 2 different type of data,
     * on the json with the info and the other the image.
     * @param run the run to be posted
     * @param mapPicture a ByteArray of the image
     * @return [Result] of type Run or a Network data error.
     */
    override suspend fun postRun(run: Run, mapPicture: ByteArray): Result<Run, DataError.Network> {
        val createRunRequestJson = Json.encodeToString(run.toCreateRunRequest())
        val result = safeCall<RunDto> {
            httpClient.submitFormWithBinaryData(
                url = constructRoute("/run"),
                formData = formData {
                    append("MAP_PICTURE", mapPicture, Headers.build {
                        append(HttpHeaders.ContentType, "image/jpeg")
                        append(HttpHeaders.ContentDisposition, "filename=mappicture.jpg")
                    })
                    append("RUN_DATA", createRunRequestJson, Headers.build {
                        append(HttpHeaders.ContentType, "text/plain")
                        append(HttpHeaders.ContentDisposition, "form-data; name=\"RUN_DATA\"")
                    })
                }
            ) {
                method = HttpMethod.Post
            }
        }
        return result.map {
            it.toRun()
        }
    }

    /**
     * Function to delete a run by id.
     * @param id the id of the Run.
     * @return [EmptyResult] with a data error in case something goes wrong.
     */
    override suspend fun deleteRun(id: String): EmptyResult<DataError.Network> {
        return httpClient.delete(
            route = "/run",
            queryParameters = mapOf(
                "id" to id
            )
        )
    }
}